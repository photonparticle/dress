...
// 	});
	...
	setTimeout(function () {
		...
		$("div.loadingOverlayInit").addClass('hidden');
...
function ajaxStart() {
	...
	$("div.loadingOverlayInit").removeClass('hidden');
}
...
function initProductSlider(slider) {
	if (slider.length > 0) {
		console.log('init slider for ' + slider.attr('class'));
		slider.flexslider({
			                  animation: "slide",
...
function validateAddToCart(product_holder) {
	...
	var form_discount_price_item = '.price_single .item_old_price span',
		...
		form_title_disabled = product_holder.find('buy-item-holder').attr('data-title-disabled');
