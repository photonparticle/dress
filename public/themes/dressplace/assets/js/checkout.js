...
function checkout(cart_holder) {
	...
	var delivery_type = cart_holder.find('div.delivery_type');
	var delivery_type_active = cart_holder.find('div.delivery_type_active');
	...
	function calculateTotals(cart_holder) {
		if (parseFloat(sub_total) >= parseFloat(delivery_free_delivery)) {
			...
			var val = cart_holder.find('div.delivery_type_active').attr('data-type');
